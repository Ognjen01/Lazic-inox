...
class PhotoSaver(
    ...
    val date = SimpleDateFormat("yyyyMMddHHmmss", Locale.getDefault()).format(Date())
        return Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM)?.absolutePath +
                "/Lazic_inox/${date}_screenshot.jpg"
    }
    ...
    private fun saveBitmapToGallery(bmp: Bitmap) {
        ...
        val contentValues = ContentValues().apply {
            ...
            put(MediaStore.MediaColumns.RELATIVE_PATH, "DCIM/Lazic inox")
        }
        ...
        activity.contentResolver.openOutputStream(uri ?: return).use { outputStream ->
            outputStream?.let {
                try {
                    saveDataToGalery(bmp, outputStream)
                } catch(e: IOException) {
                    Toast.makeText(activity, "Došlo je do greške.", Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun saveDataToGalery(bmp: Bitmap, outputStream: OutputStream){
        ...
        PixelCopy.request(arSceneView, bmp,{ result ->
            if(result == PixelCopy.SUCCESS){
                ...
                activity.runOnUiThread {
                    Toast.makeText(activity, "Uspješno sačuvana fotografija!", Toast.LENGTH_LONG).show()
                }
            } else {
                activity.runOnUiThread {
                    Toast.makeText(activity, "Došlo je do greške!", Toast.LENGTH_LONG).show()
